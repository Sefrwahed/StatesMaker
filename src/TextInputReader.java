...
public class TextInputReader
{
	...
	/**
	 * @param path the location of the input txt file
	 */
	...
	/**
	 * @param file The location of the inputs txt file
	 * @return Reads the input file and returns a string just like the original txt file
	 * @throws IOException
	 */
	...
	{
		this.ff_type = (fullInput.substring(fullInput.indexOf("FF_TYPE:")+8, fullInput.indexOf("FF_NO:")-1)).trim();
		this.ff_no = Integer.parseInt(((fullInput.substring(fullInput.indexOf("FF_NO:")+6, fullInput.indexOf("INPUT_NO:")-1)).trim()));
		this.input_no = Integer.parseInt(((fullInput.substring(fullInput.indexOf("INPUT_NO:")+9, fullInput.indexOf("OUTPUT_NO:")-1)).trim()));
		this.output_no = Integer.parseInt(((fullInput.substring(fullInput.indexOf("OUTPUT_NO:")+10, fullInput.indexOf("INTERNAL_NO:")-1)).trim()));
		this.internals_no = Integer.parseInt(((fullInput.substring(fullInput.indexOf("INTERNAL_NO:")+12, fullInput.indexOf("INPUTS:")-1)).trim()));
		String all_inputs = new String((fullInput.substring(fullInput.indexOf("INPUTS:")+7, fullInput.indexOf("OUTPUTS:")-1)).trim());
		String all_outputs = new String((fullInput.substring(fullInput.indexOf("OUTPUTS:")+8, fullInput.indexOf("INTERNALS:")-1)).trim());
		String all_internals = new String((fullInput.substring(fullInput.indexOf("INTERNALS:")+10, fullInput.indexOf("DESIGN:")-1)).trim());
		String all_design = new String((fullInput.substring(fullInput.indexOf("DESIGN:")+7)).replaceAll(" ", ""));
		...
		while (true)
		{
			...
			{
				this.inputs.add(all_inputs.substring(0, all_inputs.indexOf(",")).trim());
		...
		while (true)
		{
			...
			{
				this.outputs.add(all_outputs.substring(0, all_outputs.indexOf(",")).trim());
				all_outputs = all_outputs.substring(all_outputs.indexOf(",")+1);
			}
			else
			{
		...
		while (true)
		{
			...
			{
				this.internals.add(all_internals.substring(0, all_internals.indexOf(",")).trim());
				all_internals = all_internals.substring(all_internals.indexOf(",")+1);
			}
			else
			{
