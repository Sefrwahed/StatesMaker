...
public class GraphViz
{
	...
	private static String DOT = "C:/Program Files (x86)/Graphviz2.36/bin/dot.exe"; // Windows
	...
	private byte[] get_img_stream(File dot, String type)
	{
		...
		try
		{
			img = File.createTempFile("tmp", ".dot.tmp", new File(TEMP_DIR));

			Runtime rt = Runtime.getRuntime();

			// patch by Mike Chenault
			String[] args =
			{ DOT, "-T" + type, dot.getAbsolutePath(), "-o",
					img.getAbsolutePath() };
	...
	private File writeDotSourceToFile(String str) throws java.io.IOException
	{
		...
		try
		{
			temp = File.createTempFile("tmp", ".dot.tmp", new File(TEMP_DIR));
