...
/**
 * The circuit which contains the flipflops, internals, inputs, outputs and
 * internals
 * 
 * @author A.Aly
 */
public class Circuit
{
	...
	/**
	 * Tries all the possible combination of flipflops states and inputs to get
	 * the state diagram
	 * 
	 * @throws Exception
	 */
	public String generateAllOutputs() throws Exception
	{
		...
		ArrayList<ArrayList<Boolean>> allStates = CircuitHelpers
				.binaryCombination(flipflops.size());
		for (int state = 0; state < allStates.size(); state++)
		{
			result += "S" + state + ":\n";

			// assign the flip flops states to this configuration
			CircuitHelpers.mapFlipFlopsCurrentStates(flipflops,
					allStates.get(state));

			// Generate and try all the possible combinations of the input
			ArrayList<ArrayList<Boolean>> allInputsCombinations = CircuitHelpers
					.binaryCombination(inputs.size());
			for (int inputCombination = 0; inputCombination < allInputsCombinations
					.size(); inputCombination++)
			{
				// Assign the input values to the current combination
				CircuitHelpers.mapInputValues(inputs,
						allInputsCombinations.get(inputCombination));
				for (Input input : inputs)
					result += input.name + "="
							+ CircuitHelpers.boolToInteger(input.getValue())
							+ " ";
				...
				for (Output output : outputs)
					result += "," + output.name + "="
							+ CircuitHelpers.boolToInteger(output.getValue())
							+ " ";
				result += "\n";
			}
		}
	...
	/**
	 * @return the state diagram Begins with the initial state of all flipflops
	 *         = 0 then tries all inputs combinations and do the same with the
	 *         state reached from the initial state
	 */
	public String generateSimplifiedOutput()
	{
		...
		// Start a DFS from the state where all the flip flops are zeroes and
		// mark it as explored
	...
	/**
	 * @param initialState
	 *            list of binary numbers representing flipflops current state
	 * @param explored
	 *            hash map containing the states already explored
	 * @param resultArr
	 *            list of the strings composing the state diagram description
	 * @param stateNames
	 *            list of names of states discovered so far
	 */
			...
			ArrayList<String> stateNames)
	{
		...
		else
		{
			...
			{
				ArrayList<Boolean> nextStateCombination = CircuitHelpers
						.decimalToBinary(nextStates.get(i), flipflops.size());
	...
	/**
	 * @param reader
	 *            TextInputReader initialized with the path of the input file
	 *            containing the circuit description Connects the components of
	 *            the Circuit object
	 */
	...
	public void drawCircuit(String output, String output_file)
	{
		...
		for (int i = 3; i < max_lines;)
		{
			...
			while (i < max_lines)
			{
				...
				// splitted_line[0] has inputs, splitted_line[1] has outputs and
				// next state.
				String label = "";
				...
				matcher = Pattern.compile("(S[0-9]+)")
						.matcher(splitted_line[1]);
				matcher.find();
